...
p_set=rand(N, 2)';  % generate N random samples of p
p_set = 2*p_set - 1;
...
for j=1:length(order_pool)
    ...
    [u_alpha, eval_col_list(j) ] = Gauss_Ledendre_quadrature_solver( R, fg, f0, order, qua_order, col_acr );  % compute the coefficients using gauss-legendre quadrature
    
    
    %[u_alpha_seidel, eval_sei_list(j)]    = Gauss_Seidel_solver( R, fg, f0, order, qua_order , jac_acr ); %   using Gauss-Seidel iterations
    
    
    [u_alpha_jacobi, eval_jac_list(j) ]    = Gauss_Jacobi_solver( R, fg, f0, order, qua_order, jac_acr ); %   using Gauss-Jacobi iterations
    ...
    % And now do it the right way
    for i=1:N
        % compare the approximations to the reference u
        col_err(i) =  sum((legendre_approx( u_alpha, ind, p_set(:,i) ) - u_ref(:,i)).^2);
        jac_err(i) =  sum((legendre_approx( u_alpha_jacobi, ind, p_set(:,i) ) - u_ref(:,i)).^2);
    end
    
    col_err_list(j)=sqrt(mean(col_err));
    jac_err_list(j)=sqrt(mean(jac_err));
...
legend(  'collocation',  'Galerkin'  ); hold off;
