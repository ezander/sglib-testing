...
N = 100;
[u_mean, u_var, minfo] = compute_moments_mc(minfo, a_alpha, V_a, N);
show_mean_var('Monte-Carlo', u_mean, u_var)

%% Quasi Monte Carlo

[u_mean, u_var, minfo] = compute_moments_mc(minfo, a_alpha, V_a, N, 'mode', 'qmc');
...
p = 5;
[u_mean, u_var, minfo] = compute_moments_quad(minfo, a_alpha, V_a, p, 'grid', 'full_tensor');
show_mean_var('Full tensor grid integration', u_mean, u_var);

%% Direct integration sparse grid

p = 5;
[u_mean, u_var, minfo] = compute_moments_quad(minfo, a_alpha, V_a, p, 'grid', 'smolyak');
...
[u_i_alpha, minfo, x, w] = compute_response_surface_projection(minfo, a_alpha, V_a, V_u, p_int, 'grid', 'full_tensor');
...
V_u = gpcbasis_create(V_a, 'p', p_u);
[u_i_alpha, minfo, x, w] = compute_response_surface_projection(minfo, a_alpha, V_a, V_u, p_int, 'grid', 'smolyak');
...
[u_i_alpha, minfo, x] = compute_response_surface_tensor_interpolate(minfo, a_alpha, V_a, V_u, p_u);
...
[u_i_alpha,minfo,x,w]=compute_response_surface_nonintrusive_galerkin(minfo, a_alpha, V_a, V_u, p_int, 'grid', 'full_tensor');
