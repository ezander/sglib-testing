...
switch solver
    case 'direct'
        [x,v]=damped_spring_solve_direct(u0, v0, m, k, d, T);
    case 'numerical'
        [x, v]=damped_spring_solve_ode(u0, v0, m, k, d, T);
    otherwise
        error('foo:bar', 'Unknown solver: %s', solver);
end

function [xt, vt]=damped_spring_solve_direct(x0, v0, m, k, d, T)
...
function [xt, vt, t, ut]=damped_spring_solve_ode(u0, m, k, d, T)
[t,ut] = ode45(@damped_spring_ode, [0, T], u0, [], m, k, d);
assert(t(end)==T)
xt = ut(end,1);
vt = ut(end,2);

function [dudt] = damped_spring_ode(~, u, m, k, d)
x = u(1);
v = u(2);
a = -(2*d*v+k*x)/m;
dudt = [v; a];
